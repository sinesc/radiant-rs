...
impl Bloom {
    pub fn new(context: &Context, dimensions: (u32, u32), iterations: u32, spread: u8, brightness: f32) -> Self {
        ...
        let blur_program = Program::from_string(&context, include_str!("blur.wgsl")).unwrap();
        let mut combine_program = Program::from_string(&context, include_str!("combine.wgsl")).unwrap();
