...
impl Scene {
    ...
    pub fn op(self: &Self, op: Op) -> OpId {
    ...
    pub fn ops(self: &Self, ops: &[Op]) -> &Self {
        for op in ops {
            self.op(*op);
        }
        self
    ...
    pub fn write<T>(self: &Self, layer_id: LayerId, font_id: FontId, text: &str, position: T) -> &Self where Vec2<f32>: From<T> {
        let layers = self.layers.get();
