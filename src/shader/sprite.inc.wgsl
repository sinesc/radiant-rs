// Sprite fragment preamble — automatically prepended to custom sprite fragment shaders.
// Provides bindings, SpriteFragmentInput, and sheet*() helpers.
// Custom shaders write: @fragment fn main(input: SpriteFragmentInput) -> @location(0) vec4<f32>
// Helpers take `input` explicitly: sheet(input, coords), sheetSize(input), sheetComponent(input, coords, component)
...
fn sheetComponent(input: SpriteFragmentInput, texture_coords: vec2<f32>, component: u32) -> vec4<f32> {
    if (input._rd_v_bucket_id == 0u) {
        ...
        return textureSample(_rd_tex1, _rd_sampler, texture_coords, i32(input._rd_v_texture_id + component));
    } else if (input._rd_v_bucket_id == 2u) {
        return textureSample(_rd_tex2, _rd_sampler, texture_coords, i32(input._rd_v_texture_id + component));
    } else if (input._rd_v_bucket_id == 3u) {
        return textureSample(_rd_tex3, _rd_sampler, texture_coords, i32(input._rd_v_texture_id + component));
    } else if (input._rd_v_bucket_id == 4u) {
        return textureSample(_rd_tex4, _rd_sampler, texture_coords, i32(input._rd_v_texture_id + component));
    } else {
        return textureSample(_rd_tex5, _rd_sampler, texture_coords, i32(input._rd_v_texture_id + component));
    }
}
