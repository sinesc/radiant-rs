    ...
    fn process(self: &mut Self, renderer: &Renderer) {
        ...
        for _ in 0..self.iterations {
            ...
            for i in 0..self.targets[1].len() {
                ...
                renderer.draw_rect((0., 0.), self.dimensions, self.iter_blend, Some(&self.blur_program), Some(&self.targets[0][i]));
            }
            ...
            for i in 0..self.targets[0].len() {
                renderer.set_target(&self.targets[0][i]);
                renderer.draw_rect((0., 0.), self.dimensions, self.iter_blend, Some(&self.blur_program), Some(&self.targets[1][i]));
            }
        }
    }
